...
def dashboard_view(request):
    if request.user.is_instructor:
        ...
        progress_percent = int((completed / total_exercises) * 100) if total_exercises else 0
        students = User.objects.filter(is_instructor=False)
        leaderboard = students.annotate(total_points=Sum("progress__score")).order_by("-total_points")[:5]
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if response.status_code == 201:
                ...
                # ✅ Fix: Clean the folder if it already exists and is not empty
                ...
                remote_with_token = f"https://{settings.GITHUB_TOKEN}:x-oauth-basic@github.com/{owner}/{repo_name}.git"
...
def mark_exercise_complete(request, exercise_id):
    ...
    progress, created = Progress.objects.get_or_create(
        user=request.user,
        exercise=exercise,
        defaults={"completed": True, "score": exercise.difficulty * 10}  # <-- calculate score immediately if new
    )

    # If it already exists and not marked complete, update it
    if not created and not progress.completed:
        progress.completed = True
        progress.score = exercise.difficulty * 10  # <-- calculate score when marking complete
        progress.save()
